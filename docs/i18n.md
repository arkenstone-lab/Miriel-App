...
There are 14 namespaces, each with a JSON file per language:
...
| `auth` | `auth.json` | Login, signup, find-id, find-password forms |
| `gamification` | `gamification.json` | Levels, badges, check-in questions |
| `settings` | `settings.json` | Language, theme, notifications, AI personalization, account, privacy, support, legal |
| `privacy` | `privacy.json` | Privacy notice title, body, acknowledge button |
| `onboarding` | `onboarding.json` | Onboarding steps, persona, complete screen |
| `legal` | `legal.json` | Terms of service, privacy policy content |
| `errors` | `errors.json` | Error messages (AUTH, ENTRY, TODO, SUMMARY, SETTINGS, PROFILE, AIPREF codes) |
| `setup` | `setup.json` | First-time setup flow (language, theme, welcome) |
...
t('date.minutesAgo', { count: 5 })  // → "5m ago"
```

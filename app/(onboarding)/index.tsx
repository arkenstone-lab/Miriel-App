...
export default function OnboardingScreen() {
  ...
  const handleNext = async () => {
    if (step < TOTAL_STEPS - 1) {
      ...
      // Step 3 complete — batch save notification settings
        ...
        <View className={`w-full items-center ${isDesktop ? 'max-w-md bg-gray-50 dark:bg-gray-900 rounded-3xl p-10 border border-gray-100 dark:border-gray-800' : ''}`}>
          ...
          {step === 2 && (
            <View className="w-full mb-6">
              ...
              {!notifEnabled ? (
                <TouchableOpacity
                  className="w-full bg-indigo-100 dark:bg-indigo-900/30 py-3 rounded-xl items-center"
                  onPress={handleRequestPermission}
                  activeOpacity={0.7}
                >
                  <View className="flex-row items-center">
                    <FontAwesome name="bell" size={16} color={isDark ? '#818cf8' : '#4f46e5'} />
                    <Text className="ml-2 text-sm font-semibold text-indigo-600 dark:text-indigo-400">
                      {t('step3.allowNotifications')}
                    </Text>
                  </View>
                </TouchableOpacity>
              ) : (

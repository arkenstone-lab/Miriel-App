...
function SummaryListCard({
  ...
  const sentenceCount =
    summary.sentences_data?.length ||
    summary.text.split("\n").filter(Boolean).length;
  const countLabel =
    period === "daily"
      ? t("daily.summaryCount", { count: sentenceCount })
      : t(period === "weekly" ? "weekly.pointCount" : "monthly.pointCount", {
          count: sentenceCount,
        });
  const dateLabel =
    period === "daily"
    ...
    >
      ...
      <View className="flex-row gap-2 mt-2.5">
        ...
        {summary.entry_links.length > 0 && (
          <Badge
            label={tCommon("label.evidenceCount", {
              count: summary.entry_links.length,
            })}
            variant="cyan"
          />
        )}
...
function getMonthlyPeriodStart(reviewDay: number): string {
  ...
  if (today >= reviewDay) {
    return formatDateStr(
      new Date(now.getFullYear(), now.getMonth(), reviewDay),
    );
  }
  return formatDateStr(
    new Date(now.getFullYear(), now.getMonth() - 1, reviewDay),
  );
}
...
export default function SummaryScreen() {
  ...
  const { username, occupation, interests, monthlyReviewDay } =
    useSettingsStore();
  ...
  const filteredSummaries = useMemo(() => {
    ...
    return summaries.filter((s) =>
      isDateInPeriod(selectedDate, s.period_start, period, day),
    );
  ...
  const hasWeeklyThisWeek =
    period === "weekly" &&
    summaries?.some((s) => s.period_start === currentWeekStart);

  // Monthly review 1-per-period limit
  const currentMonthStart = getMonthlyPeriodStart(monthlyReviewDay || 1);
  const hasMonthlyThisPeriod =
    period === "monthly" &&
    summaries?.some((s) => s.period_start === currentMonthStart);
  ...
  const generateLabel =
    period === "daily"
      ? generateMutation.isPending
        ? t("daily.generating")
        : t("daily.generateButton")
      : period === "weekly"
        ? hasWeeklyThisWeek
          ? t("weekly.alreadyGenerated")
          : generateMutation.isPending
            ? t("weekly.generating")
            : t("weekly.generateButton")
        : hasMonthlyThisPeriod
          ? t("monthly.alreadyGenerated")
          : generateMutation.isPending
            ? t("monthly.generating")
            : t("monthly.generateButton");
  ...
  const emptyTitle =
    period === "daily"
      ...
      : period === "weekly"
        ? t("weekly.emptyTitle")
        : t("monthly.emptyTitle");
  const emptyDesc =
    period === "daily"
      ? t("daily.emptyDescription")
      : period === "weekly"
        ? t("weekly.emptyDescription")
  ...
  const listData =
    viewMode === "calendar" ? filteredSummaries : summaries || [];
  ...
  const master = (
    <View className="flex-1 bg-gray-50 dark:bg-gray-950">
      <FlatList
        ...
        ListHeaderComponent={
          <View className="px-4 pt-4 pb-6 gap-3" style={{ gap: 12 }}>
